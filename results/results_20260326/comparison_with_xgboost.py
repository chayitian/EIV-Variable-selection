...
from src.XGBoost_Corrected_Lasso import XGBoostCorrectedLasso
...
def monte_carlo_evaluation(n_simulations, n, p, s, alpha, sigma=1.0, sigma_u=0.5):
    ...
    models = {
        ...
        'XGBoost Corrected Lasso': XGBoostCorrectedLasso(alpha=alpha, Sigma_uu=Sigma_uu, n_estimators=50, max_depth=5, learning_rate=0.1, importance_type='gain', weight_method='normalized')
    }
    ...
    for i in range(n_simulations):
        ...
        models['XGBoost Corrected Lasso'] = XGBoostCorrectedLasso(alpha=alpha, Sigma_uu=Sigma_uu, n_estimators=50, max_depth=5, learning_rate=0.1, importance_type='gain', weight_method='normalized')
...
def main():
    ...
    with open(os.path.join(save_dir, f'all_results_xgb_{timestamp}.pkl'), 'wb') as f:
