...
class OLS:
    ...
    def __init__(self, fit_intercept=True, normalize=False, solver='solve',
    ...
    def _solve_linear_system(self, Sigma_X, rho):
        ...
        if self.solver == 'gd':
            ...
            if self.gd_lr is None:
                lipschitz = max(np.max(np.linalg.eigvalsh(Sigma_X)), 1e-12)
                lr = 1.0 / lipschitz
            else:
                if self.gd_lr <= 0:
