...
def tune_random_forest_and_get_importance(w, y, seed, cv_folds=5):
...
def fit_rf_with_best_params_and_get_importance(w, y, rf_params, seed):
    ...
    rf.fit(w, y)
    return normalize_feature_importance(rf.feature_importances_)
...
def build_model(method_name, alpha, sigma_uu, init_coef=None):
    ...
    if method_name in ('ACoCoLasso', 'RFACoCoLasso'):
...
def main():
    ...
    print('=' * 90)
    print('RFACoCoLasso reproduction started')
        ...
        for struct_idx, (struct_key, struct_label) in enumerate(structures):
            ...
            for tau_idx, tau in enumerate(taus):
                for method_idx, method in enumerate(methods):
                    ...
                    elif method == 'RFACoCoLasso':
                        rf_best_params, rf_cv_loss, init_coef_cv = tune_random_forest_and_get_importance(
                            w=w_cv,
                            y=y_cv,
                            seed=cv_seed,
                    ...
                    if method == 'ACoCoLasso':
                        ...
                        print(
                            f'Running method={method}, tau={tau}, rf_cv_loss={rf_cv_loss:.6f}, '
                            f'best_alpha={best_alpha:.6f}, cv_loss={cv_loss:.6f}, '
                            f'rf_best_params={rf_best_params}'
                        )
                    else:
                        print(
                    ...
                    for sim in range(n_simulations):
                        ...
                        try:
                            ...
                            if method == 'ACoCoLasso':
                                ...
                                init_coef_sim = fit_rf_with_best_params_and_get_importance(
                                    w=w,
                                    y=y,
