...
def compute_shap_init_coef_from_cocolasso(init_model, w):
    try:
        explainer = shap.LinearExplainer(init_model, w)
        if hasattr(explainer, 'shap_values'):
            shap_result = explainer.shap_values(w)
        else:
            shap_result = explainer(w)
    except Exception as exc:
        raise RuntimeError(f'Failed to compute SHAP from CoCoLasso: {exc}') from exc

    shap_values = _to_shap_matrix(shap_result)
    if shap_values.shape[1] != w.shape[1]:
        raise ValueError(
    ...
    w_std = np.std(w, axis=0, ddof=0)
    w_std_safe = np.where(w_std > 1e-12, w_std, 1.0)
    init_coef = shap_importance / w_std_safe
    init_coef = np.where(w_std > 1e-12, init_coef, 0.0)
...
def main():
    ...
    print('=' * 90)
    print('ShapACoCoLasso reproduction started')
    ...
    for exp_idx, exp in enumerate(experiments):
        ...
        for struct_idx, (struct_key, struct_label) in enumerate(structures):
            ...
            csv_path = output_dir / f"ShapACoCoLasso_reproduction_{exp['name']}_{struct_label}.csv"
